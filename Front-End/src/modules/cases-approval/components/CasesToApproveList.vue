<template>
	...
	<teleport to="body">
		<CaseSuccessCard
			v-if="approvalSuccessVisible && !!approvedCaseData"
			:visible="approvalSuccessVisible"
			:case-data="approvedCaseData"
			mode="created"
			@close="approvalSuccessVisible = false"
		/>
	</teleport>
...
import CaseSuccessCard from '@/shared/components/ui/feedback/CaseSuccessCard.vue'
...
import { ApprovalAdapter, type CaseToApproveViewModel } from '../services/approval.adapter'
import { useToasts } from '@/shared/composables/useToasts'

const searchTerm = ref('')
const loading = ref(false)
const errorMessage = ref('')
const cases = ref<CaseToApproveViewModel[]>([])
const total = ref(0)
...
const approvedCaseData = ref<any | null>(null)
const approvalSuccessVisible = ref(false)

// Debug helper: permitir abrir el modal manualmente desde consola
if (typeof window !== 'undefined') {
	;(window as any).openApprovalSuccessModal = (data?: any) => {
		approvedCaseData.value = data || {
			original_case_code: '2025-00001',
			code: '2025-00099',
			patient: { name: 'Paciente de prueba', patient_code: '123456' },
			complementary_tests: [ { code: 'IHC', name: 'Inmunohistoquímica', quantity: 1 } ],
			created_at: new Date().toISOString(),
			priority: 'normal',
			state: 'approved'
		}
		approvalSuccessVisible.value = true
		console.log('[Approval][Debug] Forzando apertura de modal de éxito')
	}
}

const statusClasses = (status: string) => ApprovalAdapter.getStatusClasses(status as ApprovalState)

const statusText = (status: string) => ApprovalAdapter.getStatusText(status as ApprovalState)
...
const fetchCases = async () => {
	...
	try {
		...
		cases.value = ApprovalAdapter.toViewModelList(dataList)
	} catch (e: any) {
...
// Watch para cambio de estado (sin debounce)
watch([selectedStatus], () => {
	currentPage.value = 1
	fetchCases()
})

// Watch para búsqueda de texto (con debounce)
watch(searchTerm, () => {
	currentPage.value = 1
...
const formatDate = (dateString: string): string => ApprovalAdapter.formatDate(dateString)

const getCaseTests = (caseItem: CaseToApproveViewModel) => caseItem.complementaryTests || []
const getPathologistName = (caseItem: CaseToApproveViewModel): string => 
...
const confirmApproveCase = async (): Promise<void> => {
	...
	try {
		...
		// Soportar ambas formas de respuesta: { data: { new_case }} o { new_case }
		const newCase: any = (result as any)?.data?.new_case ?? (result as any)?.new_case

		if (newCase) {
			// Agregar el código del caso original al nuevo caso
			const caseDataWithOriginal = {
				...newCase,
				original_case_code: caseItem.caseCode,
				complementary_tests: caseItem.complementaryTests
			}
			console.log('[Approval] Mostrando modal de éxito con new_case:', caseDataWithOriginal)
			showSuccessNotification(caseDataWithOriginal)
		} else {
			// Fallback: mostrar modal con datos básicos cuando no se generó el nuevo caso
			const fallbackData = {
				original_case_code: caseItem.caseCode,
				patient: {
			...
			console.warn('[Approval] new_case no disponible, mostrando modal con datos básicos:', fallbackData)
			showSuccessNotification(fallbackData)
		}
...
const showSuccessNotification = (caseData: any) => {
	approvedCaseData.value = caseData
	approvalSuccessVisible.value = true
	console.log('[Approval] Modal de éxito visible:', approvalSuccessVisible.value, 'Datos:', approvedCaseData.value)
}
</script>
