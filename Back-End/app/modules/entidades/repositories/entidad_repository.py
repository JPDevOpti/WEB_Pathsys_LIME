...
class EntidadRepository:
    ...
    async def create(self, entidad_data: EntidadCreate) -> Entidad:
        ...
        entidad_dict["fecha_creacion"] = datetime.now(timezone.utc)
    ...
    async def get_by_code(self, code: str) -> Optional[Entidad]:
        ...
        entidad_doc = await self.collection.find_one({"entidad_code": code})
        if entidad_doc:
    ...
    async def delete_by_code(self, code: str) -> Optional[Entidad]:
        """Eliminar una entidad por código (eliminación permanente)"""
        result = await self.collection.delete_one({"entidad_code": code})
