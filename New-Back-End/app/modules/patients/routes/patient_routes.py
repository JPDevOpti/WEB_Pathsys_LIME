...
from app.modules.patients.schemas import PatientCreate, PatientUpdate, PatientResponse, PatientSearch, Gender, CareType
...
async def advanced_search(
    ...
    try:
        search_params = PatientSearch(
            name=name, patient_code=patient_code, age_min=age_min, age_max=age_max,
            entity=entity, gender=gender, care_type=care_type,
...
@router.put("/{patient_code}/change-code", response_model=PatientResponse)
async def change_patient_code(patient_code: str, new_code: str = Query(...), service: PatientService = Depends(get_service)):
    try:
        return await service.change_patient_code(patient_code, new_code)
    except NotFoundError as e:
        raise HTTPException(status_code=404, detail=str(e))
    except ConflictError as e:
        raise HTTPException(status_code=409, detail=str(e))
